...
Microsoft-Windows-MediaEngine  |  127       |  Media Foundation MediaEngine  |  PresentAt({object}): QueueStats Len({Length}) Queued({Queued}) Free({QueuedWithRepeats}) DeltaToTarget({EstimatedFramesNeeded} ms) SubmittedAheadDelta({Free} ms) TimeInQueue({DeltaToTarget} ms) MinLatency({SubmittedAheadDelta} ms) MaxLatency({TimeInQueue} ms) FramePeriod ({object}0 us)
