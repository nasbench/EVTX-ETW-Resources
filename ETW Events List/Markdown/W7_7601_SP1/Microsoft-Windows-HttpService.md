...
Microsoft-Windows-HttpService  |  16        |  HTTP Service Channel  |  Response (request pointer {RequestObj}, corresponding to request ID {RequestId}, site ID {SiteId}, number of bytes {BytesSent}, endoding {Encoding}) queued for sending from the cache.
