...
def convertCsvToMD(fileName, folderName):
    ...
    with open("CSV/" + folderName + "/Providers/" + fileName + ".csv", "r") as f:
        table = csvtomd.csv_to_table(f, ",")
    
    if table:
        with open("Markdown/" + folderName + "/Providers/" + fileName + ".md", "w") as f:
            f.write(
...
def convertCsvToMDStats(folderName):
    csvReadme = "CSV/" + folderName + "/" + "README.csv"
    with open(csvReadme, "r") as f:
        table = csvtomd.csv_to_table(f, ",")
    
    if table:
        with open("CSV/" + folderName + "/" + "README.md", "w") as f:
            try:
                f.write("# Windows " + folderName.split("_")[0].split("W")[1] + " - " + folderName.split("_")[1] + "(" + folderName.split("_")[2] + ")  - ETW Providers\n\n")
            except:
                f.write("# Windows " + folderName.split("_")[0].split("W")[1] + " - " + folderName.split("_")[1] + "  - ETW Providers\n\n")
            f.write(
                csvtomd.md_table(table, padding=2)
            )
        os.remove(csvReadme)
        copyfile("CSV/"+folderName+"/README.md", "Markdown/"+folderName+"/README.md")
    else:
        print("Cannot Convert to Markdown")

# Function to generate stats readme
def getNumberOfEventsPerVersion(listOfAllFiles):
    ...
    for i in listOfAllFiles:
        ...
        version = "Windows " + fullName[0].split("W")[1]
        build = False
        try:
            build = fullName[2]
        except:
        ...
        with open(i, "r") as f:
            ...
            if build:
                folderName = fullName[0] + "_" + fullName[1] + "_" + build
            else:
                folderName = fullName[0] + "_" + fullName[1]
                
            if not os.path.exists("CSV/" + folderName):
                os.mkdir("CSV/" + folderName)
            else:
                pass
            with open("CSV/" + folderName + "/README.csv", "w") as ff:
                ff.write("ETW Provider,Number Of Events\n")
                for i in Counter([i.split(",")[0] for i in allevents]).most_common():
                    ff.write(i[0] + "," + str(i[1]))
                    ff.write("\n")

            convertCsvToMDStats(folderName)

            numOfProviders = len(Counter([i.split(",")[0] for i in allevents ]))
        final_restls.append(version + "," + update + ","+ str(numOfProviders) + "," + str(numOfEvents))

    with open("etw-stats.csv", "w") as fff:
            fff.write("Name,Version,N° Manifests,Number Of Events\n")
            for i in final_restls:
                fff.write(i)
                fff.write("\n")
    # Convert Results to MD
    with open("etw-stats.csv", "r") as f:
        table = csvtomd.csv_to_table(f, ",")
    with open("etw-stats.md", "w") as f:
        f.write(csvtomd.md_table(table, padding=2))
        os.remove("etw-stats.csv")
...
def converterStart(parsed_provider, folderName):
    ...
    with open("CSV/" + folderName + "/Providers/" + parsed_provider["provider_name"] + ".csv", "w") as f:
        f.write(header)
        f.write("\n")
...
def merge_csv(folderName):
    header = "Provider,Level,Event ID,Version,Channel,Task,Opcode,Keyword,Message"
    path_to_csv_folder = "CSV/" + folderName + "/Providers/"
...
if __name__ == "__main__":
    ...
    os.mkdir("CSV/" + folderName + "/Providers")
    os.mkdir("Markdown/" + folderName + "/Providers")
